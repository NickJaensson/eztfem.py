...
def basis_triangle_P2(xr):
    ...
    for dm in range(2):
        ...
        dphi[:, 1, dm] = 4 * (dlam[:, 0, dm] * lam[:, 1] + lam[:, 0] * dlam[:, 1, dm])
        dphi[:, 2, dm] = (4 * lam[:, 1] - 1) * dlam[:, 1, dm]
        dphi[:, 3, dm] = 4 * (dlam[:, 1, dm] * lam[:, 2] + lam[:, 1] * dlam[:, 2, dm])
        dphi[:, 4, dm] = (4 * lam[:, 2] - 1) * dlam[:, 2, dm]
        dphi[:, 5, dm] = 4 * (dlam[:, 2, dm] * lam[:, 0] + lam[:, 2] * dlam[:, 0, dm])
...
def basis_triangle_P2plus(xr):
    ...
    for dm in range(2):
        ...
        dphi[:, 0, dm] = (4 * lam[:, 0] - 1) * dlam[:, 0, dm] + 3 * dbubble[:, 0, dm]
        dphi[:, 1, dm] = 4 * (dlam[:, 0, dm] * lam[:, 1] + lam[:, 0] * dlam[:, 1, dm]) - 12 * dbubble[:, 0, dm]
        dphi[:, 2, dm] = (4 * lam[:, 1] - 1) * dlam[:, 1, dm] + 3 * dbubble[:, 0, dm]
        dphi[:, 3, dm] = 4 * (dlam[:, 1, dm] * lam[:, 2] + lam[:, 1] * dlam[:, 2, dm]) - 12 * dbubble[:, 0, dm]
        dphi[:, 4, dm] = (4 * lam[:, 2] - 1) * dlam[:, 2, dm] + 3 * dbubble[:, 0, dm]
        dphi[:, 5, dm] = 4 * (dlam[:, 2, dm] * lam[:, 0] + lam[:, 2] * dlam[:, 0, dm]) - 12 * dbubble[:, 0, dm]
