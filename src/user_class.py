...
class User:
    ...
    def __eq__(self, other):
        ...
        check0.append(('phi' in attributes_self) is ('phi' in attributes_other) )
        check0.append(('psi' in attributes_self) is ('psi' in attributes_other) )
        ...
        if 'xr' in attributes_self:
            check1.append(np.allclose(np.squeeze(self.xr),other.xr,atol=1e-15,rtol=0))
        if 'phi' in attributes_self:
            check1.append(np.allclose(np.squeeze(self.phi),other.phi,atol=1e-15,rtol=0))
        if 'psi' in attributes_self:
            check1.append(np.allclose(np.squeeze(self.psi),other.psi,atol=1e-15,rtol=0))
        if 'dphi' in attributes_self:
            check1.append(np.allclose(self.dphi,other.dphi,atol=1e-15,rtol=0))
