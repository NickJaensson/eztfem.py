...
class TestPytfem(unittest.TestCase):
    ...
    def test_poisson1(self):
        ...
        self.assertTrue(isclose(results[0], 4.071378136849546e-07, abs_tol=eps),
                        'poisson1 failed test!')

    # value from eztfem.m (Matlab): 0.294830659717740
    def test_poisson2(self):
        results = poisson2.main()
        self.assertTrue(isclose(results[0],  0.29483065971774913, abs_tol=eps),
                        'poisson2 failed test!')

    # value from eztfem.m (Matlab): 0.149343721153131 
    def test_poisson3(self):
        results = poisson3.main()
        self.assertTrue(isclose(results[0], 0.14934372115313188, abs_tol=eps),
                        'poisson3 failed test!')

    # value from eztfem.m (Matlab): 6.522070279402215e-07
    def test_poisson4(self):
        results = poisson4.main()
        self.assertTrue(isclose(results[0], 6.522070278291991e-07, abs_tol=eps),
                        'poisson4 failed test!')

    # value from eztfem.m (Matlab): 25.865650243176205
    def test_stokes1(self):
        results = stokes1.main()
        self.assertTrue(isclose(results[0], 25.865650243176205, abs_tol=eps),
                        'stokes1 failed test!')

    # value from eztfem.m (Matlab): 0.083333333333333
    def test_stokes2(self):
        results = stokes2.main()
        self.assertTrue(isclose(results[0], 0.0833333333333339, abs_tol=eps),
                        'stokes2 failed test!')

    # value from eztfem.m (Matlab): 0.008522786557203416
    def test_streamfunction1(self):
        results = stokes1.main()
        results = streamfunction1.main(results[1], results[2], results[3])
        self.assertTrue(isclose(results[0], 0.008522786557203416, abs_tol=eps),
                        'streamfunction1 failed test!')
        
    # # value from eztfem.m (Matlab): 0.031248765345553
    # def test_gn_channel(self):
    #     results, _, _, _ = gn_channel.main()
    #     self.assertTrue(isclose(results, 0.031248765345550343, abs_tol=eps),
